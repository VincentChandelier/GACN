...
def compute_metrics(
    ...
    metrics["mse"] = (org - rec).pow(2).mean().item()
...
########original padding
    ...
    _, _, height, width = x_padded.size()##获取padding之后图像的高度和宽度
    x_padded = img_window_partition(x_padded, p)##将图像reshape，（1,C,H,W)->(N,C,Hpatch,Wpatch)保证patch一样大
    enc_time = 0
    dec_time = 0
    bpp = 0
    ...
    with open(csvfile, 'a+') as f:
        row = [imgpath[-1], bpp * num_pixels, num_pixels, bpp, metrics["mse"],
               metrics["psnr"], enc_time, dec_time]
        write = csv.writer(f)
        write.writerow(row)
    print('bpp:{}, PSNR: {}, encoding time: {}, decoding time: {}'.format(bpp, metrics["psnr"], enc_time, dec_time))
    return {
        "psnr": metrics["psnr"],
        "bpp": bpp,
